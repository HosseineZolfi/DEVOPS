...
def confirm()
    ans = input(Do you want to continue unlocking and unpausing paused instances (yesno) )
    if ans.lower() != yes
...
def wait_for_unlock(instance_id, name, max_attempts=15)
    for attempt in range(max_attempts)
        ...
        if locked == False
            ...
            return True
        time.sleep(2)
    print(f⚠️ WARNING {name} is still LOCKED after {max_attempts2} seconds.)
    return False
...
def main()
    ...
    while True
        ...
        print(fn Found {len(paused)} paused instances to process {paused})
        for instance_id in paused
            processed.add(process_instance(instance_id))
            time.sleep(3)

        print(n Re-checking paused instances...)

    final_report()
    print(n🎉 All pause-locked instances have been processed.)
